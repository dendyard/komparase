<!DOCTYPE html>
<html lang="en">

<head>
    
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
<meta name="description" content="">
<meta name="author" content="">
<title>Komparase.com</title>
<link rel="icon" type="image/png" sizes="20x20" href="<?=base_url()?>favicon2.ico">

<meta name="msapplication-TileColor" content="#c7b198">
<meta name="theme-color" content="#c7b198">
<meta name="msapplication-TileImage" content="<?=base_url()?>favicon2.ico">
    

<link href="https://fonts.googleapis.com/css2?family=Raleway:wght@200;300;400;500&display=swap" rel="stylesheet">
<link href="<?=base_url()?>assets/css/main.css?v=1" rel="stylesheet">
<link href="<?=base_url()?>assets/css/general.css?v=1" rel="stylesheet">
    
    
</head>
<body>
<div id='main'>
    <div id='main-logo'>
        <a href='<?=base_url()?>'><img src="<?=base_url()?>assets/images/mainlogo.png"></a>
    </div>
    <div id='main-search'>
        <input placeholder="Cari artikel/produk favorit kamu"
                                 type="text" class="search-main" id='searchbox'>
    </div>
</div>
    
<nav id='main-menu' class="main-menu">
    <div class="menu-container">
        <div class="menu-item-cont"><a href="#">Kategori</a></div>
        <div class="menu-item-cont"><a href="#">Produk Populer</a></div>
        <div class="menu-item-cont"><a href="#">Review</a></div>
        <div class="menu-item-cont"><a href="#">Komparasi Populer</a></div>
        <div class="menu-item-cont"><a href="#">Video</a></div>
        <div class="menu-item-cont"><a href="#">Official Store</a></div>
        <div class="menu-item-cont"><a href="#">Kontak Kami</a></div>
    </div>
</nav>

    
    